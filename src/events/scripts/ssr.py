...
from events.pipeline.processors.ssr import EventSSRProcessor
...
async def run():
    async with get_chain_async("ethereum") as chain:
        start = time.time()
        await delete_key(EventSSRProcessor.redis_key)
        to_block = await chain.get_latest_block()
        await EventSSRProcessor(chain, to_block=to_block).sync()
        end = time.time()
